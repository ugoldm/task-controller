...
### Скачать бэкап на ноутбук

Выполняется **на ноутбуке** (в локальном терминале, не на сервере). `scp` использует тот же
SSH-доступ, что и подключение к серверу — отдельный пароль не нужен. Кавычки вокруг путей с `*`
обязательны, чтобы `*` раскрыл сервер, а не локальный shell.

Посмотреть список бэкапов на сервере:
```bash
ssh root@<IP> "ls -lh ~/task-controller-backups/"
```

Скачать все бэкапы в «Загрузки»:
```bash
scp "root@<IP>:~/task-controller-backups/tc-*.sqlite" ~/Downloads/
```

Скачать самый свежий одной командой:
```bash
scp "root@<IP>:$(ssh root@<IP> 'ls -t ~/task-controller-backups/tc-*.sqlite | head -1')" ~/Downloads/
```

Посмотреть содержимое скачанного файла:
```bash
DB_PATH=~/Downloads/tc-XXXX.sqlite npm run db:dump   # из папки проекта
# либо открыть в DB Browser for SQLite
```
...
> Совет: регулярно забирай свежий бэкап на свой компьютер (команды выше) — на случай отказа всего VPS.
